...
import { AVALANCHE, getConstant, getHighExecutionFee } from "config/chains";
...
export function useMinExecutionFee(library, active, chainId, infoTokens) {
  ...
  // if gas prices on Avalanche are high, the main transaction costs would come from the L2 gas usage
...
export function useStakedGmxSupply(library, active) {
  const gmxAddressEth = getContract(AVALANCHE, "GMX");
  const stakedGmxTrackerAddressEth = getContract(AVALANCHE, "StakedGmxTracker");

  const { data: ethData, mutate: ethMutate } = useSWR(
    [`StakeV2:stakedGmxSupply:${active}`, AVALANCHE, gmxAddressEth, "balanceOf", stakedGmxTrackerAddressEth],
    {
...
export function useHasOutdatedUi() {
  const url = getServerUrl(AVALANCHE, "/ui_version");
...
export function useGmxPrice(chainId, libraries, active) {
  const avalancheLibrary = libraries && libraries.avalanche ? libraries.avalanche : undefined;
...
function useGmxPriceFromEthereum() {
  const poolAddress = getContract(AVALANCHE, "UniswapGmxEthPool");
  ...
  const ethAddress = getTokenBySymbol(AVALANCHE, "WETH").address;
  const { data: ethPrice, mutate: updateEthPrice } = useSWR(
    [`StakeV2:avaxPrice`, AVALANCHE, vaultAddress, "getMinPrice", ethAddress],
    {
...
// use only the supply endpoint on avalanche, it includes the supply on avalanche
export function useTotalGmxSupply() {
  const { data: gmxSupply } = useSWR(
    [`StakeV2:totalSupply:${AVALANCHE}`, AVALANCHE, getContract(AVALANCHE, "GMX"), "totalSupply"],
    {
      fetcher: contractFetcher(undefined, Token),
    }
  );
...
export function useTotalGmxInLiquidity() {
  let poolAddressEth = getContract(AVALANCHE, "UniswapGmxEthPool");
  let totalGMX = useRef(bigNumberify(0));

  const { data: gmxInLiquidityOnEth, mutate: mutateGMXInLiquidityOnEth } = useSWR(
    [`StakeV2:gmxInLiquidity:${AVALANCHE}`, AVALANCHE, getContract(AVALANCHE, "GMX"), "balanceOf", poolAddressEth],
    {
      fetcher: contractFetcher(undefined, Token),
    }
  );
...
function useGmxPriceFromAvalanche(library, active) {
  const poolAddress = getContract(AVALANCHE, "UniswapGmxEthPool");
  const { data: uniPoolSlot0, mutate: updateUniPoolSlot0 } = useSWR(
    [`StakeV2:uniPoolSlot0:${active}`, AVALANCHE, poolAddress, "slot0"],
    {
  ...
  const ethAddress = getTokenBySymbol(AVALANCHE, "WETH").address;
  const { data: ethPrice, mutate: updateEthPrice } = useSWR(
    [`StakeV2:ethPrice:${active}`, AVALANCHE, vaultAddress, "getMinPrice", ethAddress],
    {
  ...
  const gmxPrice = useMemo(() => {
    if (uniPoolSlot0 && ethPrice) {
      const tokenA = new UniToken(AVALANCHE, ethAddress, 18, "ETH", "ETH");

      const gmxAddress = getContract(AVALANCHE, "GMX");
      const tokenB = new UniToken(AVALANCHE, gmxAddress, 18, "SEVEN", "SEVEN");
...
export function useTradersData({ from = FIRST_DATE_TS, to = NOW_TS } = {}) {
  ...
  const data = closedPositionsData
    ? sortBy(closedPositionsData.data.tradingStats, (i) => i.timestamp).map((dataItem) => {
...
export function useVolumeData({ from = FIRST_DATE_TS, to = NOW_TS } = {}) {
  const PROPS = "margin liquidation swap mint burn".split(" ");
  // const timestampProp = chainName === "avalanche" ? "id" : "timestamp"
