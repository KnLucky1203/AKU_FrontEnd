...
import { avalancheReferralsGraphClient } from "lib/subgraph/clients";
...
export function useUserCodesOnAllChain(account) {
  ...
  useEffect(() => {
    async function main() {
      ...
      const [codeOwnersOnEthereum = []] = await Promise.all([getCodeOwnersData(AVALANCHE, account, avalancheCodes)]);

      setData({
        [AVALANCHE]: codeOwnersOnEthereum.reduce((acc, cv) => {
          acc[cv.code] = cv;
          return acc;
        }, {}),
      });
