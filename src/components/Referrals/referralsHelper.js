...
export async function getReferralCodeTakenStatus(account, referralCode, chainId) {
  ...
  const [ownerETH] = await Promise.all([getReferralCodeOwner(AVALANCHE, referralCodeBytes32)]);
  const takenOnETH =
    !isAddressZero(ownerETH) && (ownerETH !== account || (ownerETH === account && chainId === AVALANCHE));

  const referralCodeTakenInfo = {
    [AVALANCHE]: takenOnETH,
    both: takenOnETH,
    ownerETH,
  };
...
export function getTwitterShareUrl(referralCode) {
  const message = ["Trying out trading on @minmaxdex, up to 50x leverage on $BTC, $ETH 📈", "For fee discounts use:"];
